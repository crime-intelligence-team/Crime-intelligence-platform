...
// ── Sample analytics dataset (static — no analytics backend endpoint exists) ──
type CorrelationCell = { row: string; col: string; value: number }
type TemporalDataPoint = { quarter: string; current: number; baseline: number; isAnomaly?: boolean }
type StatisticalOutlier = { id: string; vector: string; timeDelta: string; description: string; severity: 'critical' | 'elevated' | 'low' }
type SectorDistribution = { name: string; value: number; color: string }

const THREAT_CATEGORIES = ['Cyber Attack','Social Unrest','Supply Chain','Data Breach','Physical Incursion','Financial Fraud']
